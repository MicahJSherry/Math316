...
chart2 = alt.Chart(cloud_compare_long).mark_bar().encode(
    ...
    title="Pittsburgh Monthly Clear vs Overcast Sky Frequency (1981-2010)",
    width=700,
    height=450
)
chart2
...
chart3 = alt.Chart(cloud_monthly).mark_arc(innerRadius=60).encode(
    ...
    title="Pittsburgh Overall Cloud Cover Distribution (1981–2010)",
    width=700,
    height=450
)
chart3
...
chart4 = alt.Chart(cloud_long).mark_point(size=80).encode(
    ...
    title="Pittsburgh Cloud Cover Scatter (1981-2010)",
    width=700,
    height=450
)
chart4
...
chart7 = alt.Chart(monthly_wind).mark_line(point=True).encode(
    ...
    width=700,
    height=450,
    title='Pittsburgh Monthly Average Wind Speeds (1981-2010)'
)
...
chart8 = alt.Chart(direction_counts_ordered).mark_arc(
    ...
    title='Pittsburgh Wind Direction Distribution (1981-2010)',
    width=700,
    height=450
)
...
chart = alt.Chart(daily_stats).mark_line().encode(
    x=alt.X("DayOfYear",title= "Day of Year"),
    y=alt.Y("HLY-TEMP-NORMAL:Q",title="Average Temp. & Dew Point (F°)")
).properties(
...
chart11 = alt.Chart(daily_df).mark_point().encode(
    x=alt.X(cols[0],title="Deviation from mean pressure"),
    y=alt.Y(cols[1],title='Average Wind Speed (mph)'),
    color="Time_Category:N"
).properties(
    title="Pressure vs Wind Speed (1981-2010)",
    width=700,
    height=450
)
chart11
...
## Clear Sky Seasonal Frequency
```{python}
# Unit conversion for cloud cover
df['HLY-CLOD-PCTCLR'] = df['HLY-CLOD-PCTCLR'] / 10.0

cloud_seasonal = df.groupby('Season')['HLY-CLOD-PCTCLR'].agg([
    'mean', 'std', 'min', 'max'
]).round(1)
...
cloud_data = df.groupby('Season')['HLY-CLOD-PCTCLR'].mean().reindex(season_order).reset_index()
chart13 = alt.Chart(cloud_data).mark_bar(color='lightblue').encode(
    x=alt.X('Season:N', sort=season_order, title='Season'),
    y=alt.Y('HLY-CLOD-PCTCLR:Q', title='Average Clear Sky (%)')
).properties(
    width=550,
    height=700,
    title='Pittsburgh Seasonal Clear Sky % (1981-2010)'
)
chart13
```
