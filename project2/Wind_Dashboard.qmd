...
# Basic bar chart: mean of avg_wind_speed_kmh by region
chart = alt.Chart(wind_df).mark_bar().encode(
    x=alt.X("region:N", title="Region"),
    y=alt.Y("avg_wind_speed_kmh:Q", title="Avg Wind Speed (km/h)"),
    color="region:N",
    tooltip=["city_name","avg_wind_speed_kmh","avg_wind_direction_deg"]
).properties(title="Average Wind Speed by Region (2024)")
chart
```

## Wind Direction vs. Speed
```{python}
scatter = alt.Chart(wind_df).mark_circle(size=90).encode(
    x=alt.X("avg_wind_direction_deg:Q", title="Direction (°)"),
    y=alt.Y("avg_wind_speed_kmh:Q", title="Speed (km/h)"),
    color="region:N",
    tooltip=["city_name","region"]
).properties(title="Wind Direction vs Speed Distribution")
scatter
```
...
temp_chart = (
    ...
    .encode(
        x=alt.X("date:T", title="Date (2024)"),
        y=alt.Y("temp_c:Q", title="Avg Temperature (°C)"),
...
## Pivot Table Example
```{python}
pivot = wind_df.pivot_table(
    values="avg_wind_speed_kmh",
    index="region",
    columns="city_name",
    aggfunc="mean"
)
