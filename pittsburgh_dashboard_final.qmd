...
base = alt.Chart(daily_stats).transform_fold(
    ...
    x=alt.X("DayOfYear", title="Day of Year", scale=alt.Scale(domain=[1, 366])),
    y=alt.Y("Value:Q", title="Average Temp. & Dew Point (F°)",scale=alt.Scale(domain=[0,90])),
...
# Define the scale for the colors
color_scale = alt.Scale(
    domain=['Temp Range (10th-90th Pctl)', 'Dew Point Range (10th-90th Pctl)'],
    range=color_map # Your desired colors
)

percentile_legend = lines.mark_area(opacity=0.3).encode(
    
    y="HLY-TEMP-10PCTL:Q",
    y2="HLY-TEMP-90PCTL:Q",
    fill=alt.Fill(
        # Set the constant value (the legend label)
        'Temp Range (10th-90th Pctl)',
        # Set the nominal type for categorical data
        type='nominal', 
        # Apply the custom scale here
        scale=color_scale,
        # Optional: Add legend title if desired
        legend=alt.Legend(title="Percentile Range") 
    ),
    #color= alt.value("steelblue") 
)
temp_area = lines.mark_area(opacity=0.3).encode(
    y="HLY-TEMP-10PCTL:Q",
    y2="HLY-TEMP-90PCTL:Q",
    fill=alt.datum('Temp Range (10th-90th Pctl)')
)
dew_area = lines.mark_area(opacity=0.3).encode(
    y="HLY-DEWP-10PCTL:Q",
    y2="HLY-DEWP-90PCTL:Q",
    fill=alt.datum('Dew Point Range (10th-90th Pctl)')
